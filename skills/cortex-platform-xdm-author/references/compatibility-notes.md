...
- Reference: IDE rule WARN-023, validated 2026-04 against the live tenant. The verified ExtraHop reference rule emits this warning intrinsically.
...
The bundled `scripts/lint_rule.py` automates this for the syntactic subset it covers (ERR-012 through ERR-018, ERR-024, INFO-012). A clean exit means those classes are absent, not that the rule will pass full Cortex validation. Schema- and dataflow-dependent classes still require bisection or live-tenant verification.
